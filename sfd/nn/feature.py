...
def _parse_network(network, outputs, inputs, pretrained, ctx):
    ...
    # print(network.get_internals())
    # mx.viz.print_summary(network, {'data': (1,3, 640, 640)})
    # mx.viz.plot_network(network, shape={'data': (1,3, 640, 640)}).render('feature', cleanup=True)
...
class FeatureExpander(SymbolBlock):
    ...
    """
    def __init__(self, network, outputs, num_filters, use_1x1_transition=True, fpn_channel=64,
        ...
        # 2-layer fpn for lowest output
        o = mx.sym.UpSampling(outputs[1], scale=2, sample_type='nearest', name='fpn_up')
        o = mx.sym.Convolution(o, num_filter=fpn_channel, kernel=(1, 1), no_bias=False,
                               name='fpn_conv', attr={'__init__': weight_init})
        # o = mx.sym.Deconvolution(outputs[1], num_filter=fpn_channel, kernel=(3, 3), 
        #                          target_shape=(160,160), stride=(2, 2), no_bias=use_bn,
        #                          name='deconv', attr={'__init__': weight_init})
